...
const Map = (props) => {
    ...
    useSignalEffect(() => {
        ...
        return dag.onUpdate((result) => {
            ...
            const selected =
                all.length === result.features.length &&
                lastSource.current === props.source.value
                    ? draw.current
                          .getSelectedIds()
                          .map((id) =>
                              all.findIndex((feature) => feature.id === id),
                          )
                    : [];
            draw.current.set(result);
            const updated = draw.current.getAll();
            draw.current.changeMode("simple_select", {
                featureIds: selected.map((idx) => updated.features[idx].id),
            });
            lastSource.current = props.source.value;
            osrm(
    ...
    useEffect(() => {
        ...
        document
            ...
            .classList.add("mapboxgl-canvas");

        map.current.on("draw.selectionchange", (e) => {
            const all = draw.current.getAll().features;
            const indices = e.features.map((selected) =>
                all.findIndex((feature) => feature.id === selected.id),
            );
        ...
        map.current.on("draw.update", (e) => {
            const index = draw.current
                .getAll()
                .features.findIndex(
                    (feature) => e.features[0].id === feature.id,
                );
            const edits = editDistance(
                props.dag.value.last().features[index].geometry.coordinates,
        ...
        map.current.on("draw.cut", (e) => {
            ...
            const featureIndex = draw.current
                .getAll()
                .features.findIndex(
                    (feature) => e.feature.properties.id === feature.id,
                );
            const pointIndex = nearestPointIndex(
                e.lngLat,
                draw.current.getAll().features[featureIndex].geometry
                    .coordinates,
            );
            ...
            switch (e.code) {
                case "KeyH":
                    ...
                    map.current.fitBounds(
                        bounds.reduce(
                            ...
                            new maplibregl.LngLatBounds(bounds.slice(0, 2)),
                        ),
                ...
                case "KeyS":
                    localStorage.setItem(
                        props.source.value,
                        JSON.stringify(props.dag.value.save()),
                    );
                    break;
                case "KeyZ":
    ...
    return html`<div
        class="vh100"
        style="width: 100%; box-sizing: border-box; border: 1vh solid black"
        ref=${mapContainer}
    />`;
};
        ...
        ? html`<ul class="Files h100 scroll-y">
              ${files.map((file) =>
                  file === selectedFile
                      ? html`<li
                            class="selected"
                            onClick=${() => (props.source.value = file)}
                            key=${file}
                        >
                            ${file}
                        </li>`
                      : html`<li onClick=${() => select(file)} key=${file}>
                            ${file}
                        </li>`,
              )}
          </ul>`
    ...
    useSignalEffect(() => {
        if (source.value) {
            fetch(`/data/${source.value}`)
                ...
                .then((collection) => {
                    dag.value = Dag(
                        localStorage.getItem(source.value),
                        collection,
                    );
                    dag.value.update();
                });
